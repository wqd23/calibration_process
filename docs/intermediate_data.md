...
### 6.1 偏压串联电阻修正：跨版本检查（待确定）

修订日期：2026-09-16

**是什么。** reader 报出的 `tel['bias']` 并不是 HK 里的电压字段本身，而是按
“监测电压 − 电流 × 串联电阻”换算的。各版本用的常量不同：

- `reader05/readout.py:209-213`（03B/05B）：499 Ω（电流换算里也含 49.9/499）；
- `reader10/read.py:61-64`（10B）：499 Ω（电流分母 548.88 ≈ 499 + 49.88）；
- `reader11/read.py:55-58`（11B）、`reader12/read.py:106-114`（12B）、
  `readerN1/read.py:179-185`（GRIDN1 四个子版本）、`reader14B/read.py:154-159`
  （14B）：`bias = sipm_voltage/1000 − 499·sipm_current·1e-6`；
- `reader07/frame_adapter.py:31-33,182`（04/07/09）：`bias = vMon − iMon·R`，
  `R` 放在 `configs/{ver}/reader.yaml`（04 = 2.1 Ω，07/09 = 1.1 Ω）。

**怎么查的。** 对每个载荷挑 1–2 个温度偏压点，用它自己的 reader 读出整段 HK，
按“最后一个 |校正值 − 文件名| < 0.3 V 的连续段”选偏压稳定平台，比较平台均值：
原始监测值 `vMon`、校正值 `bias`、文件名里的设定值（如 `_290` = 29.0 V）。
同时看同一支路 4 个通道的散开：如果校正后 4 通道才对齐到设定值，说明 PID 的
反馈点在 SiPM 端（电阻之后）；如果原始值就对齐、且 4 通道原始值几乎相同
（与各通道电流无关），说明监测的是公共供电/设定值。

**结果（高温 + 高偏压点，电流最大，差异最明显；低温点两者都≈设定值）。**

| 载荷 | 文件名 | raw vMon | 校正 bias | 电流 | 谁贴设定值 | 四通道 raw 散开 | 四通道 corr 散开 |
|---|---|---|---|---|---|---|---|
| 03B | 29.0 | 29.178 | 29.0007 | 356 µA | 校正值 (0.7 mV) | 10 mV | 1.5 mV |
| 05B | 29.0 | 29.189 | 28.9973 | 384 µA | 校正值 (2.7 mV) | 27 mV | 1.2 mV |
| 04 | 29.0 | 29.570 | 28.9724 | 0.285 A* | 校正值 (28 mV) | 235 mV | 19 mV |
| 07 | 29.0 | 29.109 | 28.9913 | 0.107 A* | 校正值 (8.7 mV) | 20 mV | 3.4 mV |
| 09 | 29.0 | 29.087 | 28.9877 | 0.090 A* | 校正值 (12 mV) | 23 mV | 4.6 mV |
| 10B | 29.0 | 29.399 | 28.9913 | 816 µA | 校正值 (8.7 mV) | 75 mV | 8.1 mV |
| 11B | 29.0 | 28.9968 | 28.7298 | 535 µA | **原始值 (3.2 mV)** | 7.7 mV | 44 mV |
| 12B | 29.0 | 29.0015 | 28.8786 | 246 µA | **原始值 (1.5 mV)** | 7.8 mV | 21 mV |
| N1/GAGG | 29.0 | 28.9901 | 28.8933 | 194 µA | **原始值 (9.9 mV)** | 6.7 mV | 9.5 mV |
| N1/CLYC | 29.0 | 28.9944 | 28.8968 | 196 µA | **原始值 (5.6 mV)** | 1.5 mV | 6.7 mV |
| 14B | 29.0 | 28.9967 | 28.8137 | 367 µA | **原始值 (3.3 mV)** | 6 mV | 33 mV |

\* 04/07/09 的 `iMon` 在 reader 里是安培（其余 grid1x 载荷是 µA），仅说明量级，
不改变比较结论。复现方法：用各版本 reader 读该点的 HK 全序列（`read_frames(kind="hk")`
或 `single_readXX` 返回的 `tel`），按上面的规则取平台均值再比较；本次的图和汇总
柱状图是本地临时产出，未入库。

**结论。** 文件名是 PID 的设定值，但两代电子学的反馈点不同：

- **03B/04/05B/07/09/10B**：PID 稳的是扣掉 I·R 后的 SiPM 端电压，所以**校正值**
  等于文件名；HK 的电压字段是电阻上游的监测值。
- **11B/12B/N1/14B**：PID 稳的是公共供电/设定值，所以**原始值**等于文件名，四个
  通道的 raw 与各通道电流无关；若硬件确实是“公共供电 + 每通道串联电阻”，则
  SiPM 端电压 = 原始值 − I·R，校正值才是真正的工作偏压。

**当前处理（待确定）。** 基于上面“raw 就贴设定值”的观察，暂时把
N1/12B/14B 的电阻项置零（`bias = vMon`，见 `readerN1/read.py:179-185`、
`reader12/read.py:106-114`、`reader14B/read.py:154-159`）；`11B` 同属第二组但
**尚未改**，`03B/05B/10B` 保持 499 Ω，`04/07/09` 保持 reader.yaml 里的
1.1/2.1 Ω。**这一决定的依据只是遥测行为，缺 14B/12B/N1 的 FEE 原理图或
供应商说明**；若确认它们的 `sipm_voltage` 是公共供电端（电阻上游），则应恢复
−I·R 项（把置零改回去），这时 SiPM 端偏压才是校正值。

**对回归的影响。** 置零后：

- `lib_reader/l1_cache.py` 的 `SCHEMA_VER_PROCESSED` 升到 6，旧的 L2 处理缓存
  全部失效重算（L1 忠实帧不受影响）。
- `tests/golden/reader/n1_clyc_wf/expected.npz` 已按新约定重新冻结（N1 的
  reader golden 本来就是自洽基准，非 legacy）。
- `tests/test_pipeline_run.py`：12B 仍跑完整 pipeline 并校验产物数量，但按
  `PENDING_BIAS` 跳过与 legacy oracle 的逐字节比对（置零会让 HK 稳定段选取和
  `tel`、TB 全局系数一起变化，不是只改一个字段；原因见本节）；09 仍严格比对。
- `tests/golden/12B/points_tb.json`、`tests/golden/GRIDN1/*/points_tb.json` 仍是
  旧偏压约定下的自洽快照（测试自包含，不读 reader），待约定确认后再重新生成。
