...
- **ch0/ch3（CLYC）**：没有 Gd K 边，可用的源锚点也只有 662/1173/1332，用**单条
  线性**（`ec_form: "linear"`）。
...
重开 Cs137（662）、Co60（1173/1332）的 ch1/ch2 窗口，ch0/ch3 不变。

### Th228（583/2614 keV）整体剔除

Th228 源在这批测量里太弱，**没有可信的峰**：单谱拟合的面积显著性 `a/a_err` 只有
1.5–2.9（对比 Cs137 是 68–98、Co60 是 16–51）；拟合图上是噪声里的窄尖刺（2614）
或连续谱上的弱肩（583），不是真正的峰。用 20 min 的 Th228 文件在 2614 窗口重试
反而更差（`a/a_err`≈0.2）。因此把 `Th228C-20m-210`、`Th228C-2m-210` 从
`energy_map`、manifest 和 `fit_range_ec_source.yaml` 里整体去掉（不再进 EC 和
分辨率）。剔除后 ch0/ch3 只剩 662（Cs137 两次）、1173、1332 共 4 个点。

### 分辨率模型：同一形式 + 非负约束

GAGG 与 CLYC 用**同一个**分辨率形式 `res = sqrt(a E^2 + b E + c) / E`，不需要分
通道换形式。但原来的 `polyfit`（对 `(res*E)^2` 做加权二次）不约束系数符号，点少时
会给出负的 a/c，曲线在高能端掉到 0（ch0/ch3 尤其明显；剔除 Th228 后 ch0/ch3 的
4 个点仍拟合出 a<0、c<0）。因此 N1 EC 用 `resolution_method: polyfit_nn`：同一条
式子，但把 a,b,c 约束在 ≥0，曲线物理、不再断开。该改动只对 `GRIDN1/EC` 生效；
legacy 版本里也有负系数，保持不变以冻结回归。
...
- TB：两数据集共 107 点位。2026-09-13 关闭 `skip_qa_fail`（原先会默认丢掉
  ...
  4.6%/4.3%/4.0%/3.6%。
- EC：源 4 点（Cs137 662 ×2、Co60 1173/1332）+ X 光 13 个能量
  （40/45/47/49/51/53/55/60/65/70/75/80/90）进入单谱拟合，其中 49/51/53 落在死区、
  不参与 E-C 拟合；Th228 整体剔除（见上）。ch1/ch2 分段二次、ch0/ch3 单条线性。
  四通道 E-C 相对偏差 max：ch0 1.1%、ch1 2.6%、ch2 4.4%、ch3 1.1%。
  ch1/ch2 的低段只有 40/45/47 三个点，正好定死一条二次；`_center_fit` 对这种
  "点数 = 阶数+1" 的情形退化为不带协方差的插值拟合（误差棒记 0，系数不受影响）。
  分辨率用 `polyfit_nn`（系数非负），ch0/ch3 的曲线物理、不再断开。N1 没有 legacy
  EC 系数可对，按物理合理性验收，并作为自洽 golden（`tests/golden/GRIDN1/EC/`）。
- 中子：20 点自洽 TB 快照（见上）。
