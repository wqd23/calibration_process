...
def parse_grid_data_new(file_name, xml_file=None, data_tag='wf_packet', 
    ...
    if xml_file is None:
        raise ValueError("xml_file is required (packet definitions are per payload)")
...
def parse_grid_data_iter(file_name, xml_file=None, data_tag='grid1x_wf_packet',
    ...
    """Yield parsed packet chunks so very large waveform files stay bounded.

    ``packet_len`` must be given for the per-payload waveform variant (e.g. the
    1080-byte N1 512-sample packet).  A truncated final packet is dropped with a
    ...
    if file_size % packet_len != 0:
        warnings.warn(
            f'{file_name}: size {file_size} not aligned to packet_len={packet_len}; '
            f'dropping trailing {file_size % packet_len} bytes (truncated final packet)'
        )
    with file_path.open('rb') as fin:
        for chunk_index, event_start in enumerate(range(0, total_events, chunk_events)):
            ...
            parsed, index = parse_grid_data_new(
                file_name=file_name, xml_file=xml_file, data_tag=data_tag,
                multi_evt=multi_evt, multi_step=multi_step, endian=endian,
                crc_check=crc_check, data=chunk_data, skip_et=skip_et,
                packet_len=packet_len,
            )
            yield {
                'data': parsed, 'index': index, 'chunk_index': chunk_index,
                'event_start': event_start, 'event_count': event_count,
