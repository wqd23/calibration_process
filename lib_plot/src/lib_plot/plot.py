...
def ec_plot(energy,center, result, src_energy, x_energy, src_result,x_result, save_path, energy_split_low, energy_split_high):
    ...
    for i in range(4):
        ...
        ax.plot(adc_low[i], energy_low[i], linestyle = '-', label = f'quadratic fit on EC data of ch{i}, < {energy_split_low}keV')
        ax.plot(adc_high[i], energy_high[i], linestyle = '-', label = f'quadratic fit on EC data of ch{i}, > {energy_split_high}keV')
    ...
    e_low = np.arange(np.min(energy), max(e_union[np.where(e_union < energy_split_low)]))
    e_high = np.arange(min(e_union[np.where(e_union > energy_split_high)]), np.max(energy))
    ...
    for i in range(4):
        ...
        ax.plot(e_low, resolution_low[i]*100, linestyle = '-', label = f'Fit on resolution data of ch{i}, < {energy_split_low}keV')
        ax.plot(e_high, resolution_high[i]*100, linestyle = '-', label = f'Fit on resolution data of ch{i}, > {energy_split_high}keV')
